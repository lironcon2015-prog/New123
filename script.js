/**
 * GYMPRO ELITE V10.9 - REFACTORED FLOW
 * Fixes: Back Button Logic, Arm Training Loop, Interruption Index Recovery + Advance
 */

let state = {
    week: 1, type: '', rm: 100, exIdx: 0, originalExIdx: null, setIdx: 0,
...
function haptic(type = 'light') {
    ...
    const patterns = { light: 20, medium: 40, success: [50, 50, 50], warning: [30, 30] };
...
function handleBackClick() {
    ...
    // תיקון לחצן חזור מבונוס לסט האחרון של התרגיל
    if (currentScreen === 'ui-extra') {
        state.log.pop();
        state.setIdx = state.currentEx.sets.length - 1; 
...
function stopRestTimer() { clearInterval(state.timerInterval); state.timerInterval = null; }
...
function handleExtra(isBonus) {
    if(isBonus) {
        ...
        // תיקון ניווט בסיום תרגילים שונים
        if (state.isInterruption) {
            resumeWorkout(); // פונקציה זו כעת מקדמת את האינדקס
        } else if (state.isArmPhase) {
            showArmSelection(); // ניתוב ישיר לבחירה ללא שאלות
        } else if (state.isFreestyle || state.isExtraPhase) {
...
function interruptWorkout() {
    if (!state.isInterruption && !state.isArmPhase && !state.isExtraPhase) {
        state.originalExIdx = state.exIdx;
    }
...
function resumeWorkout() {
    ...
    // שחזור האינדקס המקורי וקידום שלו ב-1 לתרגיל הבא
    if (state.originalExIdx !== null) {
        state.exIdx = state.originalExIdx + 1;
        state.originalExIdx = null;
    }
    checkFlow();
}
...
function showExerciseList(muscle) {
    ...
    filtered.forEach(ex => {
        ...
        btn.onclick = () => {
            ...
            state.currentEx = JSON.parse(JSON.stringify(dbRef)); state.currentExName = ex.name;
...
function finish() {
    ...
    navigate('ui-summary');
    let summaryText = `GYMPRO ELITE SUMMARY\n${workoutNames[state.type] || state.type} | Week: ${state.week} | ${state.workoutDurationMins}m\n\n`;
    let grouped = {};
    state.log.forEach(e => {
        if(!grouped[e.exName]) grouped[e.exName] = { sets: [], vol: 0 };
        if(!e.skip) { grouped[e.exName].sets.push(`${e.w}kg x ${e.r} (RIR ${e.rir})`); grouped[e.exName].vol += (e.w * e.r); }
    });
